...
let imageSize = Image.resolveAssetSource(ImageResources.panel);
const ratio = imageSize.height / imageSize.width;

export default class PanelScene extends PureComponent {
    ...
    componentDidMount(): void {
        ...
        this.setState({deviceId: params.deviceId, stype: obj.stype, keys: obj.keys});
    ...
    formatTransform = () => {
        let speed = this.state.obj.speed || 0;
        let angle = -140;
        if (speed >= 100) angle = 140;
        else {
            angle = angle + Math.round(speed / 100 * 280);
        }
        let rad = angle  * Math.PI / 180;
        let translateX = Math.sin(rad) * 36;
        let translateY = (Math.cos(rad) - 1) * 36;
        return {transform: [{rotate: angle + 'deg'},{translateX: translateX},{translateY: translateY}]};
    }
    ...
    render() {
        ...
        let needleStyle = this.formatTransform();
        return (
            <View style={styles.container}>
                <SpacingView height={50} />
                <ImageBackground source={ImageResources.panel} resizeMode={'stretch'} style={styles.panel}>
                    <SpacingView height={90} />
                    <MyText style={styles.speed}>{obj.speed || 0}</MyText>
                    <MyText style={styles.unit}>km/h</MyText>
                    <Image source={ImageResources.needle} resizeMode={'stretch'} style={[styles.needle, needleStyle]}/>
                </ImageBackground>
                <SpacingView height={47} />
                <View style={commonStyle.rowView}>
                    {this.renderItem(obj.distance, '里程', 'km')}
                ...
                <View style={commonStyle.rowView}>
                    ...
                    {this.renderItem(obj.total, this.state.keys.total)}
                </View>
                <SpacingView height={47} />
                <View style={commonStyle.rowView}>
                    {btn}
